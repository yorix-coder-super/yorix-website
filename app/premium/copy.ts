...
export type PremiumCopy = {
  ...
  currency: { label: string; note: string };
  request: {
    title: string;
    body: string;
    plan: string;
...
const ru: PremiumCopy = {
  ...
  hero: {
    badge: 'Yorix Premium · доступ на неделю, месяц или год',
    ...
    primary: (price) => `Открыть Premium от ${price}`,
    oneOff: 'Разовая оплата, без автопродления',
    nextLive: 'Вход через Apple → карта на защищённой странице WebPay → Premium включится в приложении сразу. Списаний потом не будет.',
  ...
  outcomes: {
    ...
    title: 'Три вещи, которые Premium делает за вас.',
    items: [
  ...
  guarantee: 'Не подошло — вернём деньги за неиспользованные дни. Без объяснений и без анкет.',
  proof: {
    eyebrow: 'Отзывы',
  ...
  plans: {
    ...
    body: 'В каждом тарифе — полный Premium: прогноз сна, коуч, аналитика и советы на день. Разовая оплата, без автопродления.',
    bestValue: 'Выгоднее всего',
    perWeek: (price) => `${price} в неделю`,
    cheaper: (pct) => `на ${pct}% дешевле недели`,
    pay: (price) => `Открыть Premium — ${price}`,
    signInToPay: 'Войти и открыть Premium',
  ...
  features: {
    ...
    body: 'Дневник сна, кормлений и ухода бесплатен всегда. Premium превращает записи в план на сегодня и ответы на ваши вопросы.',
  ...
  steps: {
    ...
    title: 'Три шага — и Premium уже в приложении.',
    items: [
      { title: 'Войдите через Apple', body: 'С тем же Apple ID, которым вы входите в Yorix на iPhone, — так Premium включится именно на вашем аккаунте.' },
      { title: 'Оплатите картой', body: 'На защищённой странице WebPay. Данные карты видит только банк; мы их не получаем и не храним.' },
      { title: 'Откройте Yorix', body: 'Premium уже включён. Письмо с датой окончания срока и чек придут на e-mail.' },
    ],
    manualTitle: 'Пока без входа через Apple? Оставьте заявку.',
    manualBody: 'Укажите почту и код аккаунта из приложения (Настройки → Аккаунт). В течение дня пришлём ссылку на оплату — до этого платить ничего не нужно. После оплаты включим Premium в течение 24 часов.',
  ...
  home: {
    nav: 'Premium',
    eyebrow: 'Premium, если App Store недоступен',
    ...
    note: 'Не подошло — вернём деньги за неиспользованные дни. Без объяснений и без анкет.',
    details: 'Условия, возврат и реквизиты продавца',
  },
  currency: { label: 'Валюта', note: 'Оплата проходит в BYN, банк карты пересчитает по своему курсу' },
  request: {
    title: 'Оформить Premium',
    body: 'Оставьте e-mail — пришлём ссылку на защищённую оплату WebPay. После оплаты включим Premium на вашем аккаунте.',
    plan: 'Срок',
    email: 'E-mail',
    code: 'Код аккаунта',
    optional: '(если уже установили приложение)',
    codeHint: 'В приложении: Настройки → Аккаунт → «Код аккаунта». Без него включим Premium по e-mail входа.',
...
const en: PremiumCopy = {
  ...
  hero: {
    badge: 'Yorix Premium · a week, a month or a year',
    ...
    primary: (price) => `Open Premium from ${price}`,
    oneOff: 'One payment, no auto-renewal',
    nextLive: "Sign in with Apple → card on WebPay's secure page → Premium switches on in the app right away. No charges later.",
    nextRequest: "Leave your e-mail — within a day we send a payment link. Nothing to pay until then.",
  },
  outcomes: {
    eyebrow: 'What changes tonight',
    title: 'Three things Premium does for you.',
    items: [
  ...
  guarantee: 'Not for you? We refund the unused days — no explanations, no forms.',
  proof: {
    eyebrow: 'Reviews',
  ...
  plans: {
    ...
    body: 'Every pass is the full Premium: sleep forecast, coach, analytics and daily advice. One payment, no auto-renewal.',
    bestValue: 'Best value',
    perWeek: (price) => `${price} a week`,
    cheaper: (pct) => `${pct}% less than weekly`,
    pay: (price) => `Open Premium — ${price}`,
    signInToPay: 'Sign in and open Premium',
  ...
  features: {
    ...
    body: 'The sleep, feeding and care diary is free forever. Premium turns the entries into a plan for today and answers to your questions.',
  ...
  steps: {
    ...
    title: 'Three steps and Premium is in the app.',
    items: [
      { title: 'Sign in with Apple', body: 'With the same Apple ID you use in Yorix on your iPhone — so Premium lands on your own account.' },
      { title: 'Pay by card', body: "On WebPay's secure page. Only the bank sees the card; we never receive or store it." },
      { title: 'Open Yorix', body: 'Premium is already on. A confirmation with the end date and a receipt arrive by e-mail.' },
    ],
    manualTitle: 'No Apple sign-in yet? Leave a request.',
    manualBody: 'Give your e-mail and the account code from the app (Settings → Account). Within a day we send a payment link — nothing to pay until then. After payment we switch Premium on within 24 hours.',
  ...
  home: {
    nav: 'Premium',
    eyebrow: 'Premium when the App Store is not an option',
    title: "Know when your baby's next sleep is due — before the fussing starts.",
    ...
    note: 'Not for you? We refund the unused days — no explanations, no forms.',
    details: 'Terms, refunds and seller details',
  },
  currency: { label: 'Currency', note: 'The card is charged in BYN; your bank converts at its own rate' },
  request: {
    title: 'Get Premium',
    body: 'Leave your e-mail — we send a secure WebPay payment link. After payment we switch Premium on for your account.',
    plan: 'Period',
    email: 'E-mail',
    code: 'Account code',
    optional: '(if you already have the app)',
    codeHint: 'In the app: Settings → Account → “Account code”. Without it we match Premium to the sign-in e-mail.',
