...
export function GiftEntryPage({ lang, locale = lang }: { lang: Lang; locale?: SiteLocale }) {
  const text = redeemText(locale, lang);
  return (
    <SubscriptionShell giftPaths={{ en: '/gift', ru: '/ru/gift' }} lang={lang} locale={locale}>
      <section className="mx-auto grid max-w-6xl gap-8 px-5 pb-20 pt-4 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-10">
        ...
        <Reveal delay={180} load>
          <div className="rounded-[2rem] border border-white/12 bg-white/[0.06] p-6 backdrop-blur-xl sm:p-8">
            ...
            <GiftCodeEntry lang={lang} locale={locale} />
          </div>
        </Reveal>
      </section>
    </SubscriptionShell>
