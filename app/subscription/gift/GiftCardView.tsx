...
export function GiftCardView({ title, message, period, eyebrow }: { title: string; message: string; period?: string; eyebrow: string }) {
  return (
    <div className="relative">
      <Tilt>
        <div className="spotlight relative isolate aspect-[3/2] w-full overflow-hidden rounded-[2rem] border border-white/12 bg-[#1E1B4B] shadow-[0_30px_90px_rgb(0_0_0/35%)]">
      ...
      <Sparkle className="-top-3 end-[14%] w-4" delay={0} />
      <Sparkle className="-bottom-2 start-[6%] w-3" delay={900} tone="lavender" />
    </div>
  );
}
